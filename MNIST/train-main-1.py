...
x = tf.compat.v1.placeholder(tf.float32, [None, 784], name='input_x')
#y_存的是实际图像的标签，即对应于每张输入图片实际的值
y_ = tf.compat.v1.placeholder(tf.float32, [None, 10], name='input_y')
...
#损失函数和损失优化
cross_entropy = tf.reduce_mean(-tf.reduce_sum(y_ * tf.math.log(y_conv)))
...
saveFile = savePath + 'itrain_cnn_net.ckpt'
...
with tf.compat.v1.Session() as sess:
    ...
    for i in range(2000):
        ...
        if i % 100 == 0:
            train_accuracy =sess.run(accuracy, feed_dict={x:batch[0], y_:batch[1], keep_prob:0.})
